...
class RBConvectionProblem(Problem):
    ...
    def define_problem(self):
        ...
        #   2. Switch the coordinate sytem to a variant of the AxisymmetricCoordinateSystem
...
def write_outfile(problem, m, outfile):
    Gamma = problem.Gamma.value
    RaC = problem.Ra.value
    outfile.write(str(Gamma) + "\t" + str(RaC) + "\t" + str(m) + "\n")
    outfile.flush()
...
with RBConvectionProblem() as problem:
    ...
    problem.initialise()
    problem.Gamma.value = 10
    outfile = open(problem.get_output_directory("bifurcation_file.txt"), "w")
    # We can pass the angular modes we want to inspect here:
    for m in [0,1,2,3]:
        problem.Ra.value = 1500
        print(problem.Ra.value)
        for param in problem.find_bifurcation_via_eigenvalues("Ra",100,do_solve=False,axial_m=[m]):
            achieved_Ra_c = True
            if param[0]>10000:
                achieved_Ra_c = False
                break
        ...
        problem.output()
        if achieved_Ra_c:
            write_outfile(problem, m, outfile)
